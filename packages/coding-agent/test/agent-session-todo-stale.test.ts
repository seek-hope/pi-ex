...
describe("stale todo steering delivery", () => {
	...
	it("delivers the warning to the agent and clears the steering queue", async () => {
		...
		// One user prompt = one turn. The warning fires once the gap reaches 5.
		for (let i = 0; i < 4; i++) {
			await session.prompt(`msg ${i}`);
		}
		expect(sawWarningText).toBeUndefined();

		await session.prompt("msg 4");
		expect(sawWarningText).toContain("todo list has not been updated for the last 5 user inputs");
	...
	it("does not count machine-delivered (extension-sourced) prompts as turns", async () => {
		...
		// 4 user prompts (count 1-4), then machine follow-ups — the count
		...
		// The next genuine user input reaches 5.
		await session.prompt("real user input");
		expect(sawWarningText).toContain("5 user inputs");
	});
});
