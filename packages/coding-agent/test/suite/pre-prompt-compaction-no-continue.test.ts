...
describe("pre-prompt compaction regression", () => {
	...
	it("compacts length-stop overflow before a new prompt without continuing from an assistant message", async () => {
		...
		harness.sessionManager.appendMessage({
			role: "user",
			// One oversized round that cannot be split (rounds are never split);
			// compaction fires pre-prompt, once the new prompt adds a second round.
		...
		expect(harness.eventsOfType("compaction_end").at(-1)).toMatchObject({
			reason: "overflow",
			aborted: false,
			// Pre-prompt compaction: the length-stopped turn was summarized away,
			// so there is no aborted turn to retry.
			willRetry: false,
		});
