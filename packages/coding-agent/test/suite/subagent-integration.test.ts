...
import { branchName, safeId } from "../../src/core/integrations/subagent/worktree.ts";
...
describe("subagent integration", () => {
	...
	it("spawns an in-process sub-agent that completes in its own worktree", async () => {
		...
		const agent = await subagent().manager.wait(details.id);
	...
	it("review returns the branch diff and merge integrates the work", async () => {
		...
		const agent = await subagent().manager.wait(id);
		expect(agent.status).toBe("done");
