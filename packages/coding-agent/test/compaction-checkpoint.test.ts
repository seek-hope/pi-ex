...
import { applyCorrections, assembleCheckpoint, compactStructured } from "../src/core/compaction/checkpoint.ts";
import type { CompactionPreparation } from "../src/core/compaction/compaction.ts";
import { emptyLedger } from "../src/core/compaction/ledger.ts";
...
describe("compactStructured", () => {
	...
	it("produces a four-layer checkpoint from two LLM passes plus the ledger", async () => {
		...
		expect(result.summary).toContain("src/cache.ts (write)");
		expect(result.summary).toContain("`pytest`");
