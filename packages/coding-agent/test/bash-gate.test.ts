...
import { checkBashGate, formatGateResponse } from "../src/core/bash-gate.ts";

// ============================================================================
// Bash Gate tests
// ============================================================================

describe("bash gate: cat / file reading", () => {
	it("blocks cat with file argument", () => {
		const m = checkBashGate("cat src/index.ts");
		expect(m).toBeDefined();
		expect(m!.rule.name).toBe("cat-file");
		expect(m!.rule.toolName).toBe("read");
	});

	it("blocks cat with flags", () => {
		const m = checkBashGate("cat -n src/index.ts");
		expect(m).toBeDefined();
		expect(m!.rule.toolName).toBe("read");
	});

	it("allows cat inside a pipeline (|)", () => {
		// Pipeline: cat file | grep pattern — cat is part of a chain, not standalone
		const m = checkBashGate("cat src/*.ts | grep 'func'");
		expect(m).toBeDefined(); // matches cat-file rule
		// The current regex catches standalone cat even with pipe — this is
		// intentional: reading a file should go through read(), whose output
		// can then be filtered in-context.
	});
	...
	it("blocks head file", () => {
		const m = checkBashGate("head -n 20 README.md");
		expect(m).toBeDefined();
		expect(m!.rule.toolName).toBe("read");
	});

	it("blocks tail file", () => {
		const m = checkBashGate("tail -n 5 server.log");
		expect(m).toBeDefined();
		expect(m!.rule.toolName).toBe("read");
	});
...
describe("formatGateResponse", () => {
	it("formats a blocked command response with reason", () => {
		...
		expect(resp).toContain("cat src/index.ts");
		expect(resp).toContain("read()");
		expect(resp).toContain("bg_spawn()");
	});

	it("truncates long commands", () => {
		const longArg = "a".repeat(150);
		const m = checkBashGate(`cat ${longArg}`);
		expect(m).toBeDefined();
		const resp = formatGateResponse(m!);
...
describe("bash gate: path-prefixed bypass", () => {
	it("blocks /bin/cat", () => {
		expect(checkBashGate("/bin/cat package.json")).toBeDefined();
	});

	it("blocks /usr/bin/cat", () => {
		expect(checkBashGate("/usr/bin/cat /etc/hosts")).toBeDefined();
	});
...
describe("bash gate: quoting/escaping bypass (false-negative fixes)", () => {
	it("blocks backslash-escaped command word", () => {
		const m = checkBashGate("\\cat file");
		expect(m).toBeDefined();
		expect(m!.rule.name).toBe("cat-file");
	});

	it("blocks single-quoted command word", () => {
		const m = checkBashGate("'cat' file");
		expect(m).toBeDefined();
		expect(m!.rule.name).toBe("cat-file");
	});

	it("blocks double-quoted command word", () => {
		const m = checkBashGate('"cat" file');
		expect(m).toBeDefined();
		expect(m!.rule.name).toBe("cat-file");
	});
...
describe("bash gate: command chaining", () => {
	...
	it("blocks cat after ;", () => {
		const m = checkBashGate("foo; cat x");
		expect(m).toBeDefined();
		expect(m!.rule.name).toBe("cat-file");
	});
	...
	it("still blocks cat as a pipe segment", () => {
		const m = checkBashGate("cat file | grep x");
		expect(m).toBeDefined();
		expect(m!.rule.name).toBe("cat-file");
	});
});
...
describe("bash gate: wrapper bypass", () => {
	it("blocks sudo cat", () => {
		expect(checkBashGate("sudo cat file")!.rule.name).toBe("cat-file");
	});

	it("blocks command cat", () => {
		expect(checkBashGate("command cat file")!.rule.name).toBe("cat-file");
	});
	...
	it("blocks xargs cat", () => {
		expect(checkBashGate("xargs cat file.txt")!.rule.name).toBe("cat-file");
	});

	it("blocks nice cat", () => {
		expect(checkBashGate("nice cat f")!.rule.name).toBe("cat-file");
	});

	it("blocks time cat", () => {
		expect(checkBashGate("time cat f")!.rule.name).toBe("cat-file");
	});
...
describe("bash gate: input redirect reads", () => {
	it("blocks cat < input.txt", () => {
		const m = checkBashGate("cat < input.txt");
		expect(m).toBeDefined();
		expect(m!.rule.name).toBe("cat-file");
		expect(m!.rule.toolName).toBe("read");
	});
	...
	it("blocks head < file", () => {
		const m = checkBashGate("head < file");
		expect(m).toBeDefined();
		expect(m!.rule.name).toBe("head-file");
	});
...
describe("bash gate: stdin pipe filters (no file operand)", () => {
	...
	it("still blocks file reads through flags", () => {
		expect(checkBashGate("cat -n file")!.rule.name).toBe("cat-file");
		expect(checkBashGate("head -n 5 file")!.rule.name).toBe("head-file");
		expect(checkBashGate("tail -n +5 file")!.rule.name).toBe("tail-file");
		expect(checkBashGate("cat file | grep x")!.rule.name).toBe("cat-file");
	});
...
describe("bash gate: pager and cat variants", () => {
	...
	it("blocks tac/bat/batcat with a file operand", () => {
		expect(checkBashGate("tac file")!.rule.name).toBe("cat-file");
		expect(checkBashGate("bat file")!.rule.name).toBe("cat-file");
		expect(checkBashGate("batcat file")!.rule.name).toBe("cat-file");
	});
