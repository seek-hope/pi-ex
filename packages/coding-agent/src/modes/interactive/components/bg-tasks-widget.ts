...
import type { BackgroundTask } from "../../../core/integrations/bg-tasks/store.ts";
...
export class BgTasksWidget extends Container implements Focusable {
	...
	constructor(opts: BgTasksWidgetOptions) {
		super();
		this.opts = opts;
	...
	refresh(): void {
		...
		this.titleText.setText(theme.fg("accent", `Background tasks (${running.length} running, ${total} total)`));
		...
		for (let i = 0; i < visible; i++) {
			...
			row.setText(selected ? theme.fg("accent", `› ${taskLine(task)}`) : `  ${taskLine(task)}`);
		}
