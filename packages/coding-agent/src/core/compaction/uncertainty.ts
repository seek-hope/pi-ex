...
export class UncertaintyStore {
	...
	scanAssistantText(text: string, messageId: string): number {
		...
		for (const flag of parseUncertainFlags(text, messageId)) {
			const key = `${flag.type}:${normalizeClaim(flag.claim)}`;
	...
	decide(
		...
		if (decision === "dismissed") {
			this.seen.delete(`${flag.type}:${normalizeClaim(flag.claim)}`);
			this.persist();
			return true;
		}
	...
	overrideDecision(
		...
		if (decision === "dismissed") {
			...
			this.seen.delete(`${prev.type}:${normalizeClaim(prev.claim)}`);
			this.persist();
			return true;
		}
		this.appendDecision(
	...
	remove(flagIdValue: string): boolean {
		...
		if (pendingIdx !== -1) {
			...
			this.seen.delete(`${f.type}:${normalizeClaim(f.claim)}`);
			this.persist();
			return true;
		}
		const prev = this.latestDecision(flagIdValue);
	...
	updateClaim(flagIdValue: string, newText: string): boolean {
		...
		if (pending) {
			this.seen.delete(`${pending.type}:${normalizeClaim(pending.claim)}`);
			pending.claim = claim;
			this.seen.add(`${pending.type}:${normalizeClaim(claim)}`);
			this.persist();
			return true;
		}
		for (let i = this.decisionLog.length - 1; i >= 0; i--) {
			...
			this.seen.delete(`${d.type}:${normalizeClaim(d.claim)}`);
			d.claim = claim;
			this.seen.add(`${d.type}:${normalizeClaim(claim)}`);
			this.persist();
			return true;
		}
		return false;
	...
	private restoreFromBranch(): void {
		...
		for (let i = branch.length - 1; i >= 0; i--) {
			...
			this.seen.clear();
			for (const f of this.pendingFlags) this.seen.add(`${f.type}:${normalizeClaim(f.claim)}`);
			for (const d of this.decisionLog) this.seen.add(`${d.type}:${normalizeClaim(d.claim)}`);
			return;
		}
	}
}
