...
import { combineUsage, completeSummarization } from "./compaction.ts";
import {
...
import { type ActionLedger, extractLedgerActions, formatLedger } from "./ledger.ts";
import { computeFileLists, serializeConversation } from "./utils.ts";
...
async function runContractPass(
	...
	const response = await completeSummarization(
		...
		{ apiKey: auth.apiKey, headers: auth.headers, env: auth.env, signal: auth.signal },
		auth.streamFn,
		auth.retry,
	);
...
export function assembleCheckpoint(options: {
	...
	const { contract, executionState, ledger } = options;
	const out: string[] = [];
	...
	out.push("\n## World State");
	if (ledgerText) out.push(ledgerText);
	if (options.readFiles.length > 0 || options.modifiedFiles.length > 0) {
		if (options.readFiles.length > 0) {
			out.push("\n### Files Read");
			for (const f of options.readFiles) out.push(`- ${f}`);
		}
		if (options.modifiedFiles.length > 0) {
			out.push("\n### Files Touched (cumulative)");
			for (const f of options.modifiedFiles) out.push(`- ${f}`);
		}
	}
	if (!ledgerText && options.readFiles.length === 0 && options.modifiedFiles.length === 0) {
...
export async function compactStructured(
	...
	// Full tool results first; degrade to capped results only when oversized.
	let serialized = serializeConversation(convertToLlm(allMessages), Number.MAX_SAFE_INTEGER);
	if (estimateSerializedTokens(serialized) > tokenBudget) {
		serialized = serializeConversation(convertToLlm(allMessages));
	}
	if (estimateSerializedTokens(serialized) > tokenBudget) {
		throw new Error(
