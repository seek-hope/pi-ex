...
export interface AutoReviewFlag {
	id: string;
	type: "inference" | "question" | "state";
	claim: string;
	subject?: string;
...
/**
 * Extract the first balanced JSON object from a model reply. Unlike a greedy/lazy
 * regex ({...} can be truncated by a nested or in-string brace), this scans forward
 * from the first '{', tracking brace depth while honoring quoted strings and escapes,
 * and returns the exact span up to the matching '}'. Returns empty string when no
