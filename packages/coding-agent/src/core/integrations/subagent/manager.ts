...
export class SubagentManager {
	...
	async spawn(options: SpawnSubagentOptions): Promise<SubAgent> {
		...
		// Atomic-ish id: monotonic counter + timestamp + random suffix so two
		// agents spawned in the same millisecond (or across module reloads that
	...
	async continueAgent(id: string): Promise<SubAgent> {
		...
		// Validate the worktree is a real git worktree (a `.git` entry — a
		// file for linked worktrees or a directory for a bare checkout — and
		// its metadata directory) and that its branch still resolves. A stale
		// meta file whose branch was deleted upstream would otherwise resume
	...
	private launch(
		...
		const model = this.resolveModel(agent.model) ?? this.ctx.getModel();
		if (!model) {
			agent.status = "error";
			...
			this.ctx.sendFollowUp?.(`[Sub-agent ${agent.id} failed to start] ${agent.error}`);
			return;
		}
		...
		agent.promise = (async () => {
			try {
				const subagentResult = await runSubagent({
					...
					// resolves (e.g. provider removed since the crash). The model
					// was already resolved above; model stays defined here.
					model,
					depth,
