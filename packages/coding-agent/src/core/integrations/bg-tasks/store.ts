...
export interface BackgroundTaskEvents {
	/** Task list changed (spawn/status/widget refresh). */
	onChange(): void;
...
export class BackgroundTaskStore {
			...
			void (async () => {
				try {
					...
					if (t && t.status === "running") {
						...
						this.emitNotify(`✗ Background task ${id} killed: exceeded its timeout`, "error");
	...
	async sync(): Promise<void> {
		...
		for (const [, task] of this.tasks) {
			...
			if (sessionExists === true) {
				this.emitNotify(`◐ Resumed polling for background task ${task.id}`, "info");
		...
		try {
			...
			for (const logFile of logFiles) {
				...
				if (sessionExists === true) {
					this.emitNotify(`◐ Recovered and resumed polling for background task ${taskId}`, "info");
