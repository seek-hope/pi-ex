...
export class SshConnectionStore {
	...
	private emitRemoteTaskMessage(lines: string[], sessionId?: string): void {
	...
	async primeSudo(conn: SshConnection, force = false, passwordKey?: string): Promise<void> {
		...
		await this.shellExec(
			conn,
			`__PI_SUDOPW='${escaped}'; sudo() { printf '%s\\n' "$__PI_SUDOPW" | command sudo -S -p '' "$@"; }; ` +
				// Export so `nohup bash -c '...'` children (the foreground/background
				// command path) inherit the helper — a plain `sudo` there would fail
				// with "a terminal is required" (no tty, no function).
				`export -f sudo; export __PI_SUDOPW`,
			15_000,
		);
	...
	async spawnRemoteBg(
		...
		while (this.spawningRemoteBg.has(bgLockKey)) {
			...
			if (existing) return { logPath: existing.logPath, pid: existing.pid, deduplicated: true };
			...
			if (pollCount > MAX_POLLS) {
				...
				if (conn && (await this.isConnected(conn.key, true))) {
					...
					try {
						...
						this.emitRemoteTaskMessage(
							[
								`[SSH background task on ${host} reached max polling duration (60 min)]`,
								`Command: ${(cmd || "(not persisted)").substring(0, 200)}`,
								`Output:\n${output.substring(0, 4000)}`,
							],
							taskSessionId,
					...
					this.emitRemoteTaskMessage(
						[
							`[SSH background task on ${host} reached max polling duration (60 min), connection lost]`,
							`Command: ${(cmd || "(not persisted)").substring(0, 200)}`,
							`Log on remote: ${logPath}`,
						],
						taskSessionId,
