...
export class SshIntegration implements CoreIntegration {
	readonly id = "ssh";
	...
	constructor(ctx: CoreIntegrationContext, store?: SshConnectionStore) {
		this.ctx = ctx;
		this.store = store ?? getSshConnectionStore();
		this.sessionId = `ssh-${Date.now().toString(36)}-${randomBytes(3).toString("hex")}`;
					...
					this.remoteBatchTimer = setTimeout(() => {
						...
						if (batches.length === 1) {
							...
							for (const b of batches) {
								merged.push(b[0]); // first line is the header
							}
	...
	async handleCommand(args: string): Promise<void> {
		...
		if (trimmed.startsWith("sudo")) {
			...
			const password = await ui.input(
				`sudo password for ${conn.alias} (memory only, never shown or persisted)`,
				undefined,
				{ masked: true },
			);
			if (!password) {
				ui.notify("Cancelled — sudo password not set.", "info");
				return;
			}
			this.store.setSudoPassword(this.sudoPasswordKey(conn.key), password);
			try {
				...
				if (check.includes("SUDO_OK")) {
					ui.notify(`sudo enabled for ${conn.alias}.`, "info");
				} else {
					ui.notify(
						`sudo validation failed on ${conn.alias} — the password may be wrong. Re-run /ssh sudo.`,
						"error",
					);
				}
			} catch (err) {
				ui.notify(
					`Failed to set up sudo on ${conn.alias}: ${err instanceof Error ? err.message : String(err)}`,
					"error",
				);
			}
			return;
		...
		const definition: ToolDefinition<typeof schema> = {
			...
			execute: async (_toolCallId, params, signal) => {
				...
				if (/\bsudo\b/.test(params.command) && !store.hasSudoPassword(this.sudoPasswordKey(conn.key))) {
					...
					store.setSudoPassword(this.sudoPasswordKey(conn.key), password);
				}
				if (/\bsudo\b/.test(params.command)) {
					try {
						await store.primeSudo(conn, false, this.sudoPasswordKey(conn.key));
					} catch (err) {
						return {
							content: [
								{
						...
						content: [
							{
								type: "text",
								text:
									`Command still running on ${conn.key} after ${Math.round(waitWindow / 1000)}s — moved to background monitoring.\n` +
									`Log: ${logPath}\n` +
