...
export class TodoIntegration implements CoreIntegration {
	...
	constructor(ctx: CoreIntegrationContext) {
		...
		this.store = new TodoStore(ctx.sessionManager, {
			onChange: () => {
				// Every todo mutation (todo_write, progress, setStatus, etc.) flows
				// through the store's onChange, so this is the single place that
				// resets the stale timer. Do not also reset lastActivityTurn inside
				// getStaleWarning's capture logic — it intentionally re-arms the
				// warning window, but the mutation timestamp lives here.
	 ...
	 */
	getStaleWarning(gap = 5): string | null {
	...
	onShutdown(): void {
		...
		const ui = this.ctx.getUI();
		// Defensive optional call: test harnesses bind minimal UI stubs
		// (e.g. only `confirm`) that lack setWidget.
		ui?.setWidget?.("todo", undefined);
