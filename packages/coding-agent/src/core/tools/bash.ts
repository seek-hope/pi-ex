		...
		exec: async (command, cwd, { onData, signal, timeout, env }) => {
			...
			const child = spawn(
				...
				commandFromStdin ? shellConfig.args : [...shellConfig.args, fullCommand],
				{
					...
					env: env ?? getShellEnv({ exposeProviderSecrets: options?.exposeProviderSecrets }),
...
function resolveSpawnContext(
	...
	const env = { ...getShellEnv({ exposeProviderSecrets }) };
...
export interface BashToolOptions {
	 ...
	 * bash children. Default: false (filtered out so the model cannot read
	 * them back via `env`/`echo $VAR`). Enable to restore the old behavior.
	 */
	exposeProviderSecrets?: boolean;
...
export function createBashToolDefinition(
	...
	const exposeProviderSecrets = options?.exposeProviderSecrets ?? false;
	const ops =
		options?.operations ?? createLocalBashOperations({ shellPath: options?.shellPath, exposeProviderSecrets });
