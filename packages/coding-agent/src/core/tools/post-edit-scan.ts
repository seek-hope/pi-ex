...
export async function runPostEditScan(
	...
	const callTool = async (name: string, args: Record<string, unknown>): Promise<string | undefined> => {
		...
		// The underlying tool is also passed AbortSignal.timeout(remaining), so it
		// is aborted at the deadline too — the race timer only hard-cuts the wait in
		// case an implementation ignores its abort signal. The scan runs strictly
		// sequentially (one in-flight call at a time), so the worst case is a single
		// still-alive tool invocation, never a pile-up.
