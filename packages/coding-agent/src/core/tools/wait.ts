...
const parameters = Type.Object({
	timeout: Type.Integer({
		description:
			"How long to wait before resuming, in seconds. Interactive sessions: up to 43200 (12 hours). Headless sessions: up to 120.",
		minimum: 1,
	}),
});

const DESCRIPTION =
	"Suspend the current turn and resume automatically after N seconds. " +
	"Use after starting a long-running background task when there is nothing useful to do right now " +
	"— instead of busy-waiting or polling. This should be the last tool call of the turn. " +
	"Background-task completions wake the agent earlier automatically. " +
