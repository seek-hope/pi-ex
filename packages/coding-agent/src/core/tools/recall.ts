...
function createRecallTool(): ToolDefinition<typeof recallSchema> {
	const definition: ToolDefinition<typeof recallSchema> = {
		...
		promptGuidelines: [
			...
			"recall searches only archived entries — content still in your active context does not need recall.",
		],
			...
			if (queries.length === 0) {
				return {
					content: [
						{
							type: "text",
							text: "Provide query, files, or entryId. Use recall_checkpoints to list past checkpoints.",
						},
					],
					details: {},
					isError: true,
