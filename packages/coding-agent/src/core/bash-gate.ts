 ...
 * provided by a safer/more-structured pi tool (read, edit, write, ssh_exec,
...
const WRAPPER_PREFIX = String.raw`^(?:(?:sudo|command|env(?:\s+\w+=\S+)*|xargs|nice|time|exec)\s+)?(?:\/(?:usr\/)?(?:local\/)?bin\/)?`;
...
const GATE_RULES: BashGateRule[] = [
	...
	// ── log following (before the plain read rules) ───────────────────
	{
		...
		category: "background",
		pattern:
			/^(?:(?:sudo|command|env(?:\s+\w+=\S+)*|xargs|nice|time|exec)\s+)?(?:\/(?:usr\/)?(?:local\/)?bin\/)?(?:tail\s+(?:(?:-[A-Za-z0-9]+|\d+)\s+)*(?:-[A-Za-z]*[fF][A-Za-z]*|--follow)(?:\s|$)|less\s+(?:-[A-Za-z]+\s+)*\+F)/m,
		toolName: "bg_spawn",
	...
	// input redirect is required. `cmd | tail -8`, `cmd | head`, `cmd | less`
	// (stdin filters, no file operand) are legitimate and stay allowed.
	{
		name: "cat-file",
		category: "read",
		pattern: new RegExp(
			WRAPPER_PREFIX + String.raw`(?:cat|tac|bat|batcat)(?:\s+-[A-Za-z]+)*(?:\s+(?![><-])\S|\s*<(?![<(])\s*\S)`,
			"m",
		),
		toolName: "read",
		reason: `"cat <file>" duplicates pi's read() tool. Use read() instead — it supports offset/limit, auto-detects images, and avoids shell escaping issues.`,
		skipIfRedirect: true,
	},
	{
		name: "head-file",
		category: "read",
		pattern:
			// Two shapes, both linear-time (no `\\s*`/lookahead-overlap):
			// R1: head <file> or head < <file> (no flags)
			// R2: flags (each with its numeric argument) then a file token.
			/^(?:(?:sudo|command|env(?:\s+\w+=\S+)*|xargs|nice|time|exec)\s+)?(?:\/(?:usr\/)?(?:local\/)?bin\/)?head\s+(?:(?![<>-])\S|<(?![<(])(?:\s+\S|\S))/m,
		toolName: "read",
		reason: `"head <file>" duplicates pi's read() tool. Use read({path, limit: N}) for the first N lines.`,
		skipIfRedirect: true,
	},
	{
		name: "head-file",
		category: "read",
		pattern:
			/^(?:(?:sudo|command|env(?:\s+\w+=\S+)*|xargs|nice|time|exec)\s+)?(?:\/(?:usr\/)?(?:local\/)?bin\/)?head(?:\s+(?:-[A-Za-z]+\s+[+-]?\d+|-\d+|\+\d+))+\s+(?:(?![<>-])\S|<(?![<(])(?:\s+\S|\S))/m,
		toolName: "read",
		reason: `"head <file>" duplicates pi's read() tool. Use read({path, limit: N}) for the first N lines.`,
		skipIfRedirect: true,
	},
	{
		...
		category: "read",
		pattern:
			/^(?:(?:sudo|command|env(?:\s+\w+=\S+)*|xargs|nice|time|exec)\s+)?(?:\/(?:usr\/)?(?:local\/)?bin\/)?tail\s+(?:(?![<>-])\S|<(?![<(])(?:\s+\S|\S))/m,
		toolName: "read",
		reason: `"tail <file>" duplicates pi's read() tool. Use read({path, offset: -N}) or combine with limit.`,
		skipIfRedirect: true,
	},
	{
		name: "tail-file",
		category: "read",
		pattern:
			/^(?:(?:sudo|command|env(?:\s+\w+=\S+)*|xargs|nice|time|exec)\s+)?(?:\/(?:usr\/)?(?:local\/)?bin\/)?tail(?:\s+(?:-[A-Za-z]+\s+[+-]?\d+|-\d+|\+\d+))+\s+(?:(?![<>-])\S|<(?![<(])(?:\s+\S|\S))/m,
		toolName: "read",
		reason: `"tail <file>" duplicates pi's read() tool. Use read({path, offset: -N}) or combine with limit.`,
		skipIfRedirect: true,
	},
	...
	// ── ssh / scp ─────────────────────────────────────────────────────
	{
		...
		category: "remote",
		pattern:
			/^(?:(?:sudo|command|env(?:\s+\w+=\S+)*|xargs|nice|time|exec)\s+)?(?:\/(?:usr\/)?(?:local\/)?bin\/)?ssh\s+(?!(?:-T\s+)?git@)(?:-[A-Za-z0-9]+\s+)*(\S+@\S+|\S+)\s/m,
		toolName: "ssh_exec",
	...
	},
	{
		...
		category: "remote",
		pattern:
			/^(?:(?:sudo|command|env(?:\s+\w+=\S+)*|xargs|nice|time|exec)\s+)?(?:\/(?:usr\/)?(?:local\/)?bin\/)?scp\s/m,
	...
	// ── polling / sleep ──────────────────────────────────────────────
	{
		...
		category: "background",
		pattern:
			/^(?:(?:sudo|command|env(?:\s+\w+=\S+)*|xargs|nice|time|exec)\s+)?(?:\/(?:usr\/)?(?:local\/)?bin\/)?sleep\s+\d/m,
		toolName: "bg_spawn",
