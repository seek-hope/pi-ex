...
> **First-time checkout before running tests:** provider model data under
> `packages/ai/src/providers/data/` is gitignored and only generated by
> `npm run build` (runs `generate-models`, which needs network access). Run
> `npm run build` once before running tests on a fresh checkout, otherwise
> tests that import model data will fail with `Cannot find module
> './data/<provider>.json'`. The `test` scripts warn about this instead of
> failing silently.
